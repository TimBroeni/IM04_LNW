...
$stmt = $pdo->prepare("SELECT t.id, t.name, COALESCE(usage_counts.usage_count, 0) AS usage_count, COALESCE(latest_event.movement, 0) AS movement, latest_event.timestamp AS event_timestamp, COALESCE(weekly_removals.weekly_removals, 0) AS weekly_removals FROM toys t LEFT JOIN ( SELECT toy_id, COUNT(*) AS usage_count FROM toy_events WHERE movement = 0 AND timestamp >= :week_start GROUP BY toy_id ) usage_counts ON usage_counts.toy_id = t.id LEFT JOIN ( SELECT te1.toy_id, te1.movement, te1.timestamp FROM toy_events te1 INNER JOIN ( SELECT toy_id, MAX(id) AS max_id FROM toy_events GROUP BY toy_id ) latest ON latest.toy_id = te1.toy_id AND latest.max_id = te1.id ) latest_event ON latest_event.toy_id = t.id LEFT JOIN ( SELECT toy_id, COUNT(*) AS weekly_removals FROM toy_events WHERE movement = 0 AND timestamp >= :week_start GROUP BY toy_id ) weekly_removals ON weekly_removals.toy_id = t.id WHERE t.household_id = :household_id ORDER BY usage_count DESC, t.name ASC, t.id ASC");
...
foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $toy) {
    $toys[] = [
        'id' => (int) $toy['id'],
        'name' => $toy['name'],
        'usage_count' => (int) ($toy['usage_count'] ?? 0),
